...
run_pseudotime_analysis <- function(train_umap_path, test_umap_path, output_prefix) {
  ...
  train_subjects_multi <- Training_set %>%
    group_by(RID) %>%
    summarise(count = n()) %>%
    filter(count > 2) %>%
    pull(RID)
  
  # Randomly select up to 10 subjects
  selected_train_subjects <- if (length(train_subjects_multi) > 0) {
    sample(train_subjects_multi, size = min(10, length(train_subjects_multi)))
  } else {
    NULL
  }
  
  # Testing set: Get subjects with >2 timepoints
  test_subjects_multi <- Testing_set %>%
    group_by(RID) %>%
    summarise(count = n()) %>%
    filter(count > 2) %>%
    pull(RID)
  ...
  # Randomly select up to 10 subjects
  selected_test_subjects <- if (length(test_subjects_multi) > 0) {
    sample(test_subjects_multi, size = min(10, length(test_subjects_multi)))
  } else {
    NULL
  }
